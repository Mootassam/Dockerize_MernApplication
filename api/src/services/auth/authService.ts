...
class AuthService {
    ...
    try {
      ...
      sendSmtpEmail.htmlContent = `
            <html>
              ...
              <h3>Your account is still waiting for activation!</h3>
              <p>You will recieve an email when it is confirmed</p>
            </html>`;
      ...
      sendSmtpEmail.params = { "subject": "Ligue Digitale", "name": email.split('@')[0] };
  ...
  static async sendPasswordResetEmail(
    ...
    try {
      ...
      link = `http://www.liguedigitale.tadeco-group.tn/auth/password-reset?token=${token}`;
    } catch (error) {
      console.error(error);
      throw new Error400(
        language,
    ...
    sendSmtpEmail.subject = "{{params.subject}}";
    sendSmtpEmail.htmlContent = "<html><body><h1>{{params.name}}</h1><h2>Enter your new password <a href={{params.link}}>here</a></h2></body></html>";
    sendSmtpEmail.sender = { "name": "No Reply", "email": "ligue-digitale@tadeco-group.tn" };
    ...
    sendSmtpEmail.headers = { "Some-Custom-Name": "unique-id-1234" };
    sendSmtpEmail.params = { "parameter": "My param value", "subject": "Ligue Digitale", "link": link, "name": user.fullName };
