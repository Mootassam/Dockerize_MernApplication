...
export default class ProduitCommandeService {
  ...
  async create(data) {
    ...
    try {
      data.adherent = await UserRepository.filterIdInTenant(data.adherent, { ...this.options, session });
      data.commandLine = await CommandLineRepository.filterIdsInTenant(data.commandLine, { ...this.options, session });
  ...
  async update(id, data) {
    ...
    try {
      ...
      for (let i = 0; i < oldCommandLines.length; i++) {
        await CommandLineRepository.findById(oldCommandLines[i], this.options).then(res => {
          if (res.product.stockable) {
            let productData = {
              stock: res.product.stock + res.quantity
            }
      ...
      for (let i = 0; i < data.commandLine.length; i++) {
        await CommandLineRepository.findById(data.commandLine[i], this.options).then(res => {
          if (res.product.stockable) {
            let productData = {
              stock: res.product.stock - res.quantity
            }
      ...
      const record = await ProduitCommandeRepository.update(
        id,
        data,
        {
          ...this.options,
  ...
  async findAndCountAll(args) {
    return ProduitCommandeRepository.findAndCountAll(
      args,
      this.options,
    );
  }
