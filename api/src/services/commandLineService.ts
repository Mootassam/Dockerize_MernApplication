...
export default class CommandLineService {
  ...
  async create(data) {
    ...
    try {
      data.product = await ProduitRepository.filterIdInTenant(data.product, { ...this.options, session });
    ...
    try {
      ...
      if (commandLine.product.stockable) {
        if (data.quantity > commandLine.quantity) {
          let productData = {
            stock: commandLine.product.stock - (data.quantity - commandLine.quantity)
          }
          await ProduitRepository.update(commandLine.product.id, productData, this.options)
        }
        else if (data.quantity < commandLine.quantity) {
          let productData = {
            stock: commandLine.product.stock + (commandLine.quantity - data.quantity)
          }
          await ProduitRepository.update(commandLine.product.id, productData, this.options)
        }
      }
      const record = await CommandLineRepository.update(
        id,
        data,
        {
          ...this.options,
  ...
  async findAndCountAll(args) {
    return CommandLineRepository.findAndCountAll(
      args,
      this.options,
    );
  }
